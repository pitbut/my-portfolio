...
интерактивный зал тренировок (таракан реально ходит есть/тренируется, тапом
ставятся препятствия, драгом рядом с ним его можно подтолкнуть), выбор трассы,
полноэкранная гонка вдоль экрана с «живым» нелинейным бегом, реакция на
громкий звук (микрофон) и удар по столу (акселерометр) с адаптивным порогом,
гонка по Bluetooth (классический RFCOMM) вдвоём на одном общем поле.
