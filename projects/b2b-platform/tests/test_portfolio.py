...
def test_executor_public_profile_shows_contact_immediately(client):
    """Заказчик выбирает среди ставок исполнителей — ему нужны их контакты
    сразу, до сделки, иначе не из чего выбирать. В отличие от заказчика
    (см. следующий тест), контакты исполнителя не гейтятся заключённой
    сделкой — исполнитель это публичный цех/завод/мастер, а не частное лицо."""
    ...
    assert "телефон".encode() in resp.data
...
def test_customer_public_profile_visible_to_executor_with_portfolio_link_from_order(client):
    ...
    assert f'/messages/u/{customer_user_id}'.encode() in resp.data
...
def test_customer_can_message_bidder_from_order_detail(client):
    region_id = _setup_customer(client, "cust3@example.com")
    service_id, _ = _setup_executor(client, "exec6@example.com", region_id)

    with client.application.app_context():
        executor_user_id = User.query.filter_by(email="exec6@example.com").first().id
    ...
    _login(client, "exec6@example.com")
    client.post(
    ...
    _login(client, "cust3@example.com")
    resp = client.get(f"/orders/{order_id}")
    assert f'/messages/u/{executor_user_id}'.encode() in resp.data
