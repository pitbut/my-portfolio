...
def min_moment_of_inertia(sec_type, dims):
    """Минимальный (по слабой оси) момент инерции сечения — нужен для
    проверки устойчивости сжатых стержней (формула Эйлера). Для круга/трубы
    момент инерции одинаков по любой оси. Для прямоугольника/короба —
    считаем в обеих ориентациях и берём меньший (приближённо для двутавра
    тоже — точный Iy требует табличных данных по сортаменту)."""
    if sec_type == 'circle':
        return circle(dims['d'])['Ix']
    if sec_type == 'pipe':
        return pipe(dims['d_out'], dims['d_in'])['Ix']
    if sec_type == 'rectangle':
        b, h = dims['b'], dims['h']
        return min(rectangle(b, h)['Ix'], rectangle(h, b)['Ix'])
    if sec_type == 'box':
        b, h, t = dims['b_out'], dims['h_out'], dims['t']
        return min(box(b, h, t)['Ix'], box(h, b, t)['Ix'])
    if sec_type == 'i_beam':
        # приближённо: I_y двутавра существенно меньше I_x; точное значение —
        # по сортаменту ГОСТ. Берём консервативную оценку через полки.
        h, b, s, t = dims['h'], dims['b'], dims['s'], dims['t']
        Iy_flanges = 2 * (t * b ** 3) / 12
        Iy_web = (h - 2 * t) * s ** 3 / 12
        return Iy_flanges + Iy_web
    raise ValueError(f"Неизвестный тип сечения: {sec_type}")
