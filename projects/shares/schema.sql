...
CREATE TABLE IF NOT EXISTS users (
  ...
  -- Email нужно подтвердить, прежде чем можно будет войти и получить акцию за
  -- регистрацию (см. handleRegister/handleVerifyEmail в worker/src/index.js) —
  -- иначе на бесплатные акции можно было бы регистрировать сколько угодно
  -- одноразовых почт.
...
-- quantity — REAL, а не INTEGER: акция за регистрацию делится дробно между
-- рефералом и пригласившим (см. handleVerifyEmail), поэтому держатель может
-- владеть дробным количеством акций (например, 0.95).
...
-- signup_bonus_shares — сколько акций начисляется за подтверждённую регистрацию.
-- referral_signup_bonus_rate — какая доля этой акции (0..1) уходит пригласившему,
-- если регистрация была по реферальной ссылке (см. handleVerifyEmail).
-- Пример: 1 акция за регистрацию и ставка 0.05 → новый участник получает 0.95,
-- пригласивший — 0.05.
INSERT OR IGNORE INTO site_settings (key, value) VALUES ('signup_bonus_shares', '1');
INSERT OR IGNORE INTO site_settings (key, value) VALUES ('referral_signup_bonus_rate', '0.05');
