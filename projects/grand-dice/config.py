...
class Config:
    ...
    # Жёсткий лимит на размер запроса (защита от заливки огромных файлов в
    # чат поддержки/чеки пополнения) — с запасом над MAX_RECEIPT_BYTES
    # (8 МБ, самый большой из лимитов в app/uploads.py).
