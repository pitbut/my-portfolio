...
Многие хостинги блокируют исходящие SMTP-соединения на бесплатном тарифе,
поэтому письма отправляются через HTTPS-API Resend, а не через smtplib.
Если RESEND_API_KEY не настроен, письма реально не отправляются — текст
пишется в лог приложения. Это позволяет разрабатывать и тестировать
регистрацию/вывод средств локально без реального email-провайдера и не
роняет прод, если администратор ещё не подключил Resend.
"""
import json
import urllib.error
...
def send_email(to, subject, body):
    ...
    показывать ли пользователю содержимое прямо на странице."""
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "grand-dice-app/1.0 (+https://casino.robutpit.com)",
        },
    )
