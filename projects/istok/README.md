...
4. После создания сервиса `istok` откройте его → **Environment** → добавьте
   переменную `DATABASE_URL` со значением Internal Connection String вашей
   PostgreSQL-базы (скопируйте со страницы этой базы в Render → Connect →
   Internal Database URL). Сохраните — сервис передеплоится.
...
6. Наполните базу данными один раз через вкладку **Shell** сервиса `istok`:
   ```bash
   python seed.py
   ```
   Повторный запуск `seed.py` пересоздаёт **все** таблицы и данные с нуля — если
   в той же базе есть другие таблицы других проектов, `seed.py` их не тронет
   (он вызывает `db.drop_all()`/`db.create_all()` только для моделей `app.models`),
   но собственные данные «Истока» пересоздаст. Выполняйте осознанно, не на
   каждом деплое.
7. Готово — сервис будет доступен по адресу вида `https://istok.onrender.com`
