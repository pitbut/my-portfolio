"""Регистрация (с подтверждением email и согласием 18+), вход, выход."""
import re
...
def _send_confirmation_email(user):
    ...
    body = (
        f"Привет, {user.name}!\n\n"
        f"Подтвердите регистрацию в «Собутыльнике», перейдя по ссылке:\n"
        f"{confirm_url}\n\n"
        f"Ссылка действительна {current_app.config['CONFIRM_TOKEN_MAX_AGE'] // 3600} часов. "
        f"Если вы не регистрировались — просто проигнорируйте это письмо."
    )
    sent = send_email(user.email, "Подтверждение регистрации — Собутыльник", body)
    return sent, confirm_url


def _flash_confirmation_result(sent, confirm_url):
    """В проде ссылку подтверждения не показываем напрямую — иначе
    подтверждение email превращается в формальность, а не в проверку, что
    человек владеет этим ящиком (доступ к сайту гейтится по
    email_confirmed, см. app/__init__.py). В DEBUG показываем для удобства
    локальной разработки без настоящего email-провайдера."""
    if sent:
        ...
        flash(
            ...
            "раз через пару минут или напишите в поддержку.",
            "error",
        )
...
def register():
    ...
    if request.method == "POST":
        ...
        age_confirmed = request.form.get("age_confirm") == "on"

        form_state = {"name": name, "email": email}

        if not name:
            flash("Введите имя.", "error")
            return render_template("auth/register.html", **form_state)
        ...
        if not age_confirmed:
            flash("Нужно подтвердить, что вам есть 18 лет.", "error")
            return render_template("auth/register.html", **form_state)
...
def login():
    ...
    if request.method == "POST":
        ...
        if user.is_blocked:
            flash("Этот аккаунт заблокирован администратором. Обратитесь в поддержку.", "error")
            return render_template("auth/login.html", email=email)
...
def confirm_email(token):
    try:
        ...
        flash("Ссылка подтверждения устарела. Запросите новую в кабинете.", "error")
        return redirect(url_for("main.index"))
    except BadSignature:
